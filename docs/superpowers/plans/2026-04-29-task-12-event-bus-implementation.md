...
- Multi-line strings to native executables (`git commit -m`) use **multiple `-m` arguments**, one per paragraph. Avoids here-string parser edge cases.
...
| `crates/event-bus/src/lib.rs` | **Create** | Single source file: crate docstring, support types, traits, concrete impls, `now_ns` helper, `#[cfg(test)] mod tests`. |

**No other files are touched.** Tests are inline in `lib.rs`; there is no `tests/` directory for this crate (spec §10 + §4.1).

---

## Why workspace `members` and `[workspace.dependencies]` both change

`Cargo.toml` was trimmed in Task 11 to keep the workspace loadable while only `crates/types` existed. Task 12 reverses that trim for its own crate (other Phase 1 crates re-add their entries in Task 13–16 as those crates appear). Independently, `parking_lot` is a new dependency introduced by this crate; centralizing the version in `[workspace.dependencies]` keeps it pinnable for Task 13–16 if any of them want a `Mutex` later. Both edits land in a single chore commit (Task 1) so the precondition for any subsequent `cargo` invocation is established once.

---

## Task 1: Workspace `Cargo.toml` — re-add member + add `parking_lot` workspace dep
...
Expected: exits 0 with no output. (If non-zero, re-check Steps 1.2/1.3 — the `crates/event-bus` directory does not yet exist, so cargo will complain. That's OK for now if the error is "manifest path does not exist"; we move to Task 2 to create it. Re-verify metadata after Task 2.)

If `cargo metadata` errors with "manifest path … does not exist", proceed — Task 2 creates the manifest.
...
git add Cargo.toml
git commit `
    -m 'chore: re-add crates/event-bus to workspace and add parking_lot dep' `
    -m 'Re-adds crates/event-bus to [workspace] members for Task 12, reversing the Task 11 trim. Other Phase 1 crates (journal, config, observability, app) re-add their member entries in Task 13-16 as those crates are created. Empty stub crates remain forbidden per PHASE_1_DETAIL_REVISION.' `
    -m 'Adds parking_lot = "0.12" to [workspace.dependencies] so the event-bus crate can use parking_lot::Mutex for the publish-path lock per spec section 4.3. parking_lot is chosen for the simpler API (no poisoning) and lighter handle, not for performance; replacement of the channel/mutex stack with a lock-free ring buffer remains gated on benchmark proof per ADR-005.' `
    -m 'Co-Authored-By: Claude <noreply@anthropic.com>'
```

---

## Task 2: Scaffold the empty `crates/event-bus` crate
...
- [ ] **Step 2.2: Write `crates/event-bus/Cargo.toml`**
...
- [ ] **Step 2.3: Write minimal `crates/event-bus/src/lib.rs`**
...
- [ ] **Step 2.5: Verify the workspace metadata now loads cleanly**
...
```powershell
git add crates/event-bus/Cargo.toml crates/event-bus/src/lib.rs
git commit `
    -m 'feat(event-bus): scaffold rust-lmax-mev-event-bus crate' `
    -m 'Empty buildable crate with rust-lmax-mev-types path-dep, crossbeam-channel, metrics, thiserror, and parking_lot runtime deps per spec section 4.2. No dev-dependencies - SmokeTestPayload reaches tests via the rust-lmax-mev-types runtime dep. Subsequent tasks fill the type definitions, trait impls, and TDD tests inline in lib.rs.' `
    -m 'Co-Authored-By: Claude <noreply@anthropic.com>'
```

---

## Task 3: Support types — `PublishAck`, `BusStats`, `BusError`
...
- [ ] **Step 3.1: Replace `lib.rs` content with imports + the three types**

```rust
// Crate-level docstring is added in Task 12. Traits, concrete impls, and tests
...
Expected: compiles. Possible warnings on unused variants (`ClockUnavailable`, `SequenceExhausted`, `Closed`) since nothing constructs them yet — these go away in Tasks 5–7. **Do not** silence them with `#[allow(dead_code)]`; same as Task 11's policy on `UnsupportedEventVersion`.
...
## Task 4: Traits + concrete struct skeletons + stub impls

**Files:**
- Modify: `crates/event-bus/src/lib.rs`

Goal: add `EventBus<T>` and `EventConsumer<T>` traits, the `PublishState` private struct, the two concrete struct shells (`CrossbeamBoundedBus<T>` and `CrossbeamConsumer<T>`), and stub `impl` blocks that all compile but `unimplemented!()` at runtime. **No `new()` constructor in this task** — it lands in Task 5 driven by T1.

This task makes the API surface complete enough for the next task to write a test that constructs the bus through `new()` (which doesn't exist yet — that's the red phase of Task 5).

- [ ] **Step 4.1: Append imports + traits + struct + stub impls to `lib.rs`**
    ...
    T: Send + 'static,
{
    fn publish(&self, _payload: T, _meta: PublishMeta) -> Result<PublishAck, BusError> {
        unimplemented!("publish lands in Task 6")
    }
...
{
    fn recv(&self) -> Result<EventEnvelope<T>, BusError> {
        unimplemented!("recv lands in Task 6")
    }

    fn try_recv(&self) -> Result<Option<EventEnvelope<T>>, BusError> {
        unimplemented!("try_recv lands in Task 6")
    }
...
Note: `CrossbeamConsumer<T>` does **not** derive `Clone` (spec §5.3, §6.1, DoD D4).

`stats()` is **fully implemented** in this task (not stubbed) because (a) it touches no mutex and is trivial to write, and (b) Tasks 6+ rely on it for assertions. The `len()` and `capacity()` getters are also fully implemented for the same reason. Only `publish`, `recv`, `try_recv` are stubs.
...
Expected: compiles. The `PhantomData` is **not** needed because `Sender<EventEnvelope<T>>` and `Receiver<EventEnvelope<T>>` already use `T` non-phantomly.
...
If clippy warns about `unimplemented!` panics in non-test code, ignore — Task 5 replaces the first stub.
...
## Task 5: TDD T1 — `new(capacity)` rejects zero capacity
...
- [ ] **Step 5.1: Append the test module skeleton + Test 1 to `lib.rs`**
...
- [ ] **Step 5.2: Run Test 1, expect FAIL (compile error)**

```powershell
cargo test -p rust-lmax-mev-event-bus new_rejects_zero_capacity
```

Expected: **compile error** — `CrossbeamBoundedBus::new` does not exist. This is the red phase. The error message will mention "no function or associated item named `new` found".

- [ ] **Step 5.3: Implement `CrossbeamBoundedBus::new`**

Add this `impl` block above the `impl EventBus<T> for CrossbeamBoundedBus<T>` block:
...
- [ ] **Step 5.4: Run Test 1, expect PASS**
...
## Task 6: TDD T2 — publish + recv basic flow with envelope preservation

**Files:**
- Modify: `crates/event-bus/src/lib.rs`

Goal: write Test 2 (`publish_assigns_sequence_nonzero_timestamp_and_preserves_envelope`) per spec §10. Implement `publish()` with the simplest direct blocking `send` (no try_send fallback yet — that's Task 7), `recv()`, `try_recv()`, and the `now_ns` helper. Verify the test passes.
...
- [ ] **Step 6.1: Append T2 to the `tests` module**
...
- [ ] **Step 6.2: Run Test 2, expect FAIL**

```powershell
cargo test -p rust-lmax-mev-event-bus publish_assigns_sequence_nonzero_timestamp_and_preserves_envelope
```

Expected: panic with `"publish lands in Task 6"` from the `unimplemented!()` stub. This is the red phase.

- [ ] **Step 6.3: Add the `now_ns` helper**

Insert above the `impl<T> CrossbeamBoundedBus<T>` block (or anywhere at module scope, but grouping it with the bus impl reads better):
...
The three rejected cases (pre-epoch, `u128 → u64` overflow, exact-zero) all map to `ClockUnavailable` per spec §7.3.

- [ ] **Step 6.4: Replace the `publish()` stub with a direct blocking-send implementation**
...
fn publish(&self, payload: T, meta: PublishMeta) -> Result<PublishAck, BusError> {
    ...
    metrics::gauge!("event_bus_current_depth").set(self.sender.len() as f64);
...
The `try_send` + `Full` fallback is **not** added in this task — Task 7 introduces it as a refactor driven by T3. For now, `send` is direct and blocking.

- [ ] **Step 6.5: Replace the `recv` and `try_recv` stubs**
...
If T2 fails on `consumed_total == 3` but `published_total == 3`, the most likely cause is the `Arc<AtomicU64>` not being properly shared — recheck that `consumer.consumed_total` and `bus.consumed_total` reference the same `Arc` instance (Step 5.3 uses `Arc::clone`).
...
git commit `
    ...
    -m 'Implements the publish path per spec section 7.3 with a direct blocking send (no try_send/Full fallback yet - that lands in Task 7 driven by T3). Adds the now_ns() helper that rejects pre-epoch, u128->u64 overflow, and exact-zero timestamps to BusError::ClockUnavailable.' `
...
## Task 7: TDD T3 — try_send + `Full` → backpressure_total fallback
...
- [ ] **Step 7.1: Append T3 to the `tests` module**
...
- [ ] **Step 7.2: Run T3, expect FAIL**

```powershell
cargo test -p rust-lmax-mev-event-bus publish_registers_backpressure_when_full_and_completes_after_recv
```

Expected: the `assert!(Instant::now() < deadline, ...)` panics with "publisher thread never registered backpressure within 3s" because the current direct `send` in `publish()` never increments `backpressure_total`. This is the red phase.

- [ ] **Step 7.3: Refactor `publish()` to the spec §7.3 try_send + Full fallback shape**
...
If T3 still hangs longer than 3s on the deadline, the most likely cause is `stats()` being implemented to acquire the publish-state mutex (deadlock). Re-verify `stats()` from Task 4 reads only the atomics + `sender.len()` and does **not** call `self.state.lock()` (spec §7.2 + DoD D8).

If T2 regresses, the most likely cause is a typo in the refactored `publish()` — recheck that the success-path counter advance and the `Ok(PublishAck)` return are unchanged from Task 6.
...
## Task 8: T4 — `publish` after consumer drop returns `Closed`

**Files:**
- Modify: `crates/event-bus/src/lib.rs`

Goal: add T4 (`publish_after_consumer_drop_returns_closed`) per spec §10. The current `publish()` from Task 7 already maps `TrySendError::Disconnected` to `BusError::Closed`, so this test is expected to pass on first run (test-first verification, not red→green TDD).

- [ ] **Step 8.1: Append T4 to the `tests` module**
...
`expect_err` is OK here because `publish()` returns `Result<PublishAck, BusError>`, and `PublishAck` does derive `Debug` (Task 3, Step 3.1). The reasoning that motivated the T1 explicit-match form does not apply.

- [ ] **Step 8.2: Run T4, expect PASS on first try**
...
If FAIL with a panic in the `unimplemented!` from Task 4 (shouldn't happen — Task 6 replaced those), recheck that Task 6 fully replaced both `recv` and `try_recv` stub bodies.

If FAIL with a different `BusError` variant, the most likely cause is the `try_send` `Disconnected` arm not being added in Task 7 — recheck Step 7.3.

- [ ] **Step 8.3: Mutation check (optional but recommended)**
...
git commit `
    ...
    -m 'No implementation change needed - the closed-channel mapping was already wired in Task 7 step 7.3 via the TrySendError::Disconnected arm and the blocking send map_err.' `
    -m 'Co-Authored-By: Claude <noreply@anthropic.com>'
```

---

## Task 9: T5 — `try_recv` empty + `recv` after bus drop

**Files:**
- Modify: `crates/event-bus/src/lib.rs`

Goal: add T5 (`try_recv_empty_returns_none_and_recv_after_bus_drop_returns_closed`) per spec §10. The `try_recv` Empty → `Ok(None)` and `recv` Disconnected → `Err(Closed)` mappings already exist from Task 6, so this is also test-first verification.

- [ ] **Step 9.1: Append T5 to the `tests` module**
...
- [ ] **Step 9.2: Run T5, expect PASS on first try**
...
If FAIL on the `consumed_total == 0` assertion after `try_recv == Ok(None)`, the most likely cause is `try_recv` incrementing the counter on the empty branch — recheck Step 6.5 and confirm the increment is **only** in the `Ok(env)` arm.
...
git commit `
    ...
    -m 'No implementation change needed - both mappings were wired in Task 6 step 6.5.' `
    -m 'Co-Authored-By: Claude <noreply@anthropic.com>'
```

---

## Task 10: T6 — invalid meta retry safety (sequence not consumed on Envelope error)

**Files:**
- Modify: `crates/event-bus/src/lib.rs`

Goal: add T6 (`publish_rejects_invalid_meta_without_consuming_sequence`) per spec §10. The "success-only sequence advance" invariant from Task 6 already guarantees this — the test verifies it via the public API.

- [ ] **Step 10.1: Append T6 to the `tests` module**
...
- [ ] **Step 10.2: Run T6, expect PASS on first try**
...
If FAIL on `ack.sequence == 0` after the retry (i.e., the retry's ack.sequence is 1), the most likely cause is `next_sequence` being advanced on the Envelope error path — recheck that the `state.next_sequence = sequence + 1;` line in `publish()` (Step 7.3) sits **after** the match block, so it only runs on success.
...
git commit `
    ...
    -m 'No implementation change needed - the success-only advance was wired in Task 6 step 6.4 and Task 7 step 7.3 by placing state.next_sequence = sequence + 1 strictly after the match block.' `
    -m 'Co-Authored-By: Claude <noreply@anthropic.com>'
```

---

## Task 11: T7 — sequence exhaustion no-wrap (white-box)

**Files:**
- Modify: `crates/event-bus/src/lib.rs`

Goal: add T7 (`sequence_exhausted_does_not_wrap`) per spec §10. White-box — relies on same-module visibility of the private `state` field on `CrossbeamBoundedBus`. The implementation already includes the `if sequence == u64::MAX → SequenceExhausted` check from Task 6 step 6.4, so this is also test-first verification.

- [ ] **Step 11.1: Append T7 to the `tests` module**
...
- [ ] **Step 11.2: Run T7, expect PASS on first try**
...
If FAIL on `consumer.try_recv == Ok(None)` (i.e., the consumer received an envelope), the most likely cause is the `if sequence == u64::MAX` check being placed **after** `seal()` rather than before. Recheck Step 6.4 / Step 7.3 — the check must be at step 4 of spec §7.3, before `seal` (step 5) and `try_send` (step 6).

- [ ] **Step 11.3: Mutation check (optional)**
...
## Task 12: Crate-level docstring + per-item docstrings
...
- [ ] **Step 12.1: Replace the `lib.rs` opening comment with the crate-level docstring**
...
//! ## Retry safety
//!
//! `state.next_sequence` advances **only on publish success**. All transient
//! errors ([`BusError::ClockUnavailable`], [`BusError::Envelope`],
//! [`BusError::Closed`] before send completion) leave the counter unchanged,
//! so the next publish attempt reuses the would-have-been sequence. This
//! is the foundation of the retry-safety invariant verified by T6.
//!
...
- [ ] **Step 12.2: Add docstrings on `EventBus` and `EventConsumer` traits**
...
- [ ] **Step 12.3: Add docstrings on `PublishAck`, `BusStats`, `BusError`**
...
- [ ] **Step 12.4: Add a docstring on `CrossbeamBoundedBus::new`**

The existing docstring from Step 5.3 already contains the substantive content (capacity == 0 rejection rationale). No further change required unless it was lost in editing — re-verify it survives.

- [ ] **Step 12.5: Verify docs render**
...
git commit `
    ...
    -m 'Crate-level docstring covers topology (each pipeline stage boundary owns one bus pair, no global queue), sequence/timestamp ownership and the clone-free publish-path contract, the parking_lot mutex serialization role, the stats() no-mutex-acquisition hard requirement, the u64::MAX exhaustion sentinel and retry-safety policies, and the #[non_exhaustive] Phase 2 extension policy per spec section 11 DoD D11.' `
...
## Task 13: Final QA — fmt, build, test, clippy + DoD walk
...
- [ ] **Step 13.4: `cargo clippy` with `-D warnings`**
...
If clippy fires `clippy::cast_precision_loss` on the `as f64` casts in `metrics::gauge!(...).set(self.sender.len() as f64)`, add a localized `#[allow(clippy::cast_precision_loss)]` either on the publish/recv methods or on the specific cast expressions, with a one-line comment citing spec §8.1: "Phase 1 capacities are well below 2^53; precision loss does not apply." **Do not** suppress the lint workspace-wide here — that decision belongs to Task 18 (CI pipeline).

If clippy fires any other lint:
- Stylistic ones (e.g. `redundant_field_names`, `needless_return`) — fix by editing.
- Semantic ones — pause and re-read the lint message; do not silence with `#[allow]` without spec-level justification.

- [ ] **Step 13.5: Walk the spec §11 DoD checklist**
...
- [ ] **Step 13.6: (Optional) Cleanup commit**

If Steps 13.1–13.4 produced any auto-formatting fixes, clippy fixes, or `#[allow]` additions:
...
- [ ] **Step 13.7: Verify Task 12 readiness for Task 13 handoff**
...
13 tasks, ~14–15 commits.

TDD discipline applied to:
- Task 5 (T1: `new(0)` reject — red is a compile error).
- Task 6 (T2: publish + recv basic flow + envelope preservation — red is `unimplemented!()` panic, green is the full publish path with direct blocking send).
- Task 7 (T3: backpressure_total via try_send + Full fallback — red is the 3s deadline trip on the assert, green is the spec §7.3 step 6 match).

Test-first verification (no red phase needed because the implementation already covers the contract by Task 7) for:
- Task 8 (T4: closed-channel mapping).
- Task 9 (T5: try_recv None + recv-after-bus-drop).
- Task 10 (T6: invalid-meta retry safety).
- Task 11 (T7: sequence-exhaustion no-wrap, white-box).
...
Workspace `Cargo.toml` precondition (member re-add + `parking_lot` workspace dep) is bundled in Task 1 as a single chore commit, mirroring Task 11's "Task 1" pattern.
