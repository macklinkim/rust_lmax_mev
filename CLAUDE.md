...
**Phase 6b: COMPLETE** (git tag: `phase-6b-complete`, pushed to `origin`)
- Production Gate scope: pre-activation signer infrastructure + boundary docs + ADR-001 describing-only amendment + `live_send=true` conditional flip + local-only relay submission dress rehearsal + non-localhost adapter Ok-path flip + Bloxroute parity + keccak bundle-byte equality + production-runtime `sign_tx` call site. **Default runtime stays fail-closed**: `wire_phase4` continues to inject `DisabledSigner`; `BundleConstructor::sign_for_outcome` returns `Err(SignerDisabled)`; `simulator_driver` skips; no submission attempt fires.
- All eight batches CLOSED via the lean-batching policy:
  - Phase 6b overview (`49123e9`) + ADR-001 Amendment 1 + boundary doc `docs/specs/phase-6b-boundary.md` at P6B-A (`2ddba8a` / `1c490de`).
  - P6B-B no-SDK `ProductionSigner` stub + structured audit log-source + Profile/KeyBackend config gating (`87d27a8` / `df96ac8`).
  - P6B-C KMS infrastructure + signer audit surface + sample Alertmanager rule (`3e803c2` / `b77241a` / `ff2edbc`).
  - P6B-CD sign-activation: `BundleTx` EIP-1559 fee fields + `alloy-rlp 0.3` unsigned/signed encoder + `k256 0.13` recovery-only carve-out per ADR-001 Amendment 2 + G2f narrow allow-list + G2g absolute ban (`c9451c2` / `a68641d` / `247e7ad`).
  - P6B-D `live_send=true` conditional flip per `(Profile::Production, KeyBackend::HsmKms, non-empty audit_key_id)` (`dc5ca55` / `36e3b5a`).
  - P6B-E1 local-only relay submission dress rehearsal: `submission_driver` + `FileJournal<SubmissionReceipt>` + dual localhost gate (`ConfigError::LiveSendRequiresLocalhostEndpoint` validate + `BundleRelayError::SubmitDisabledNonLocalhost` adapter runtime) (`4ca6abd` / `78380ca`).
  - P6B-E2 non-localhost Ok-path flip + Bloxroute parity + keccak G12 step 6 + production-runtime `sign_tx` call site: `RelayConfig::allow_non_localhost_endpoint` default `false`; shared `crates/relay-clients/src/send_bundle.rs` helper for both adapters; `submission_driver` keccak compare with `SubmissionReceipt::local_bundle_hash`; new `BundleConstructor::sign_for_outcome` with G11 grep gate flipped 1 -> 2 (`8b8f8d8` / `5b52a77`).
  - P6B-F DoD audit + audit-note-1..4 follow-ups + 3 RUSTSEC-2026 rustls-webpki ignores in `deny.toml` (reachability analysis recorded inline) + `CLAUDE.md` wrap-up + `phase-6b-complete` annotated tag.
- 20 workspace crates: Phase 6a 20 unchanged. Phase 6b touched only `crates/config`, `crates/bundle-relay`, `crates/relay-clients`, `crates/app`, `crates/execution`, `crates/signer`. Other crates byte-frozen from Phase 6a.
- 280 workspace tests passing + 1 ignored live-smoke env-contract stub (P2-C carry-forward).
- Hard P6b invariants verified at `phase-6b-complete`: G2g 0 hits absolute (no funded key / `SigningKey` / 64-hex literal); G2a 0 hits (no forbidden signer-symbol set in `*.rs`); G3 = 1 (single `.submit_bundle(` callsite at `crates/app/src/lib.rs:1905` inside `submission_driver`); G4 = 5 (within CW-3 <=12 bound); G11 = 2 (test-only `invoke_signer_for_test` at `crates/execution/src/lib.rs:235` + production `sign_for_outcome` at `:278`); G14 production runtime = 1 (shared `crates/relay-clients/src/send_bundle.rs:120`; Bloxroute reuses the same helper module so no second runtime literal); G15 audit-once-per-attempt preserved (only `BundleConstructor::sign_for_outcome` reaches `signer.sign_tx`; `submission_driver` does NOT re-invoke).
- Default fail-closed posture at `phase-6b-complete`: `RelayConfig::allow_non_localhost_endpoint = false` -> `LiveSendRequiresLocalhostEndpoint` reject preserved + adapter runtime `SubmitDisabledNonLocalhost` preserved; `RelayConfig::key_backend = Disabled` -> `wire_phase4` injects `DisabledSigner` -> `sign_for_outcome` returns `Err(SignerDisabled)` -> `simulator_driver` iteration-skip; Bloxroute `submit_bundle` Ok-path also requires ctor-time `api_key` (third fail-closed layer `Err(SubmitHttpFailed)`); `BundleConstructor::sign_for_outcome` builds a placeholder `BundleTx::new(from=Address::ZERO, ...)` so even an operator-wired real `ProductionSigner` rejects with `Err(AddressMismatch)` (2-step safety chain documented in `phase-6b-boundary.md` Section 3 P6B-F NOTE-4).
- ADR-001 Amendments 1 + 2 are the only ADR amendments ever made in Phase 6b. No widening of asset (WETH/USDC), venue (UniV2 + UniV3 0.05% + Sushi V2), or V3 fee-tier scope.
- `deny.toml` carries 3 new `RUSTSEC-2026-0098/0099/0104` ignores for `rustls-webpki 0.101.7` pulled transitively via `aws-smithy-http-client 1.1.12 -> rustls 0.21.12`; reachability analysis recorded in `deny.toml` comments + revisit trigger pinned to `aws-sdk-kms` / `aws-smithy-http-client` shipping with `rustls-webpki >= 0.103.12`.
- `master` and `phase-6b-complete` tag pushed to `origin`.

**Production live action is not the default at `phase-6b-complete`.** The unlock path for a real submission is operator-driven and out of scope for this workspace's CI: (a) operator wires `ProductionSigner::from_aws_kms` with a real KMS key and a properly-populated `BundleTx` (not the `Address::ZERO` placeholder); (b) operator sets `relay.live_send = true`, `active_profile = "production"`, `relay.key_backend = "hsmkms"`, `relay.audit_key_id = "<key>"`, `relay.allow_non_localhost_endpoint = true` in the deployment TOML overlay; (c) operator points `relay.enabled_relays[0].endpoint` at a real (non-localhost) Flashbots or Bloxroute endpoint; (d) Bloxroute additionally requires `relay.enabled_relays[0].api_key`. Until all of (a)..(d) are configured, the workspace at `phase-6b-complete` fails closed at the very first gate.
...
2. Phase 1/2/3/4/5/6a/6b closed at `phase-1-complete`/`phase-2-complete`/`phase-3-complete`/`phase-4-complete`/`phase-5-complete`/`phase-6a-complete`/`phase-6b-complete`. Do not re-open frozen crates or specs without an ADR/spec change. The P3-E ADR-006 deferral, P4-G `prefetch_for` deferral, Phase 5 Safety Gate scope, Phase 6a Pre-Production Gate scope, and Phase 6b Production Gate scope (with the explicit fail-closed-by-default posture preserved) are documented in their respective phase tag annotations.
3. **Phase 6b is COMPLETE** with the workspace at fail-closed default. Any further widening (asset/venue/V3 fee-tier expansion, multi-relay parallel submission, real production `BundleTx` shape replacing the `Address::ZERO` placeholder, automation of operator-side live wiring) requires a NEW phase scope with its own ADR amendment + boundary doc + user authorization.
4. Use `superpowers:subagent-driven-development` for any future implementation work once a plan is user-approved.
...
## Task Checklist (Phase 6b — all CLOSED)

- [x] P6B-A: Phase 6b overview (`49123e9`) + ADR-001 Amendment 1 (describing-only Phase 6a/6b split) + boundary doc `docs/specs/phase-6b-boundary.md` (`2ddba8a` / `1c490de`).
- [x] P6B-B: no-SDK `ProductionSigner` stub + structured audit log-source + Profile/KeyBackend config gating (Section 2.5 candidate #4 piece) (`87d27a8` / `df96ac8`).
- [x] P6B-C: KMS infrastructure (aws-sdk-kms + aws-config dep edges into `crates/signer`) + signer audit surface (`production_signer_boot` info event + `production_signer_audit_attempts_total{outcome}` counter + threshold gauges) + sample Alertmanager rule at `config/examples/signing-audit-alert.yaml` (`3e803c2` / `b77241a` / `ff2edbc`).
- [x] P6B-CD: sign-activation — `BundleTx` EIP-1559 fee fields + `alloy-rlp 0.3` unsigned/signed encoder + `k256 0.13` recovery-only carve-out per ADR-001 Amendment 2 + G2f narrow allow-list (`k256::ecdsa::{Signature, RecoveryId, VerifyingKey}`, `k256::PublicKey`) + G2g absolute ban (no signing-key constructors / no test-key literals anywhere) + 7-outcome audit-attempts label set `{not_configured, address_mismatch, invalid_bundle_tx, kms_sign_failed, invalid_signature_bytes, signature_recovery_failed, ok}` (`c9451c2` / `a68641d` / `247e7ad`).
- [x] P6B-D: `live_send=true` conditional flip — `ConfigError::LiveSendRequiresProductionProfile` + `LiveSendRequiresHsmKms`; single legal combo `(Profile::Production, KeyBackend::HsmKms, non-empty audit_key_id)` (`dc5ca55` / `36e3b5a`).
- [x] P6B-E1: local-only relay submission dress rehearsal — NEW `submission_driver` in `wire_phase4` parallel to `comparator_driver`; NEW `FileJournal<SubmissionReceipt>` + `JournalConfig::submission_journal_path`; dual localhost gate (`ConfigError::LiveSendRequiresLocalhostEndpoint` validate + `BundleRelayError::SubmitDisabledNonLocalhost` adapter runtime); Flashbots Ok-path activation under localhost only; Bloxroute unchanged at E1 (`4ca6abd` / `78380ca`).
- [x] P6B-E2: non-localhost adapter flip + Bloxroute parity + keccak G12 step 6 + production-runtime `sign_tx` call site — `RelayConfig::allow_non_localhost_endpoint` (default `false`); BREAKING ctor sig `::new(cfg, kill_switch, allow_non_localhost)`; shared `crates/relay-clients/src/send_bundle.rs` helper; `BundleHashMismatch` reserved variant + `SubmissionReceipt::local_bundle_hash` field; NEW `BundleConstructor::sign_for_outcome` invoked from `simulator_driver` (G11 grep gate flipped 1 -> 2); `DEFAULT_FLASHBOTS_ENDPOINT` + `DEFAULT_BLOXROUTE_ENDPOINT` narrowed to `pub(crate) + #[doc(hidden)]` (`8b8f8d8` / `5b52a77`).
- [x] P6B-F: DoD audit + 4 audit-note follow-ups (NOTE-1 Bloxroute api_key contract; NOTE-2 BundleHashMismatch reserved-not-returned doc-comment; NOTE-3 D-T-E2-3 mismatch assertion strengthening; NOTE-4 placeholder BundleTx 2-step safety chain documented) + 3 new RUSTSEC-2026 advisory ignores in `deny.toml` (with reachability analysis per advisory + revisit trigger) + `CLAUDE.md` wrap-up + `phase-6b-complete` annotated tag.
